...
### Requirement: Bifurcación para targets no-web
Si `web_endpoints.txt` queda vacío (target sin servicios web), el sistema SHALL omitir automáticamente los Steps 4, 5, 6, 7 y 8 (Nuclei, Nikto, WhatWeb, Gobuster/Dirsearch, SSLyze), documentar cada salto en `manifest.json` con estado `skipped_no_web`, y continuar el flujo de análisis sobre los servicios no-web detectados por Nmap.

#### Scenario: Target sin servicios web
- **WHEN** tras el Step 2, `web_endpoints.txt` está vacío
- **THEN** los Steps 4 a 8 se marcan `skipped_no_web` en el manifest y no se ejecutan, mientras los hallazgos de servicios no-web (Step 3) siguen al Agente 3

#### Scenario: Target con servicios web
- **WHEN** tras el Step 2, `web_endpoints.txt` contiene al menos una URL
- **THEN** los Steps 4 a 8 se ejecutan normalmente sobre esos endpoints
...
### Requirement: Steps web sobre endpoints confirmados
El sistema SHALL ejecutar Nuclei, Nikto, WhatWeb y Gobuster/Dirsearch (Steps 4-7) únicamente sobre las URLs de `web_endpoints.txt`, y SSLyze (Step 8) únicamente sobre los puertos con esquema `https://`. Nikto, Gobuster/Dirsearch y SSLyze SHALL ejecutarse de forma secuencial por endpoint/puerto, guardando un archivo de evidencia por cada uno (ej. `nikto_<port>.json`, `gobuster_<port>.txt`, `sslyze_<port>.json`).
...
Al terminar el pipeline de un target, el sistema SHALL generar `evidence/<target>/manifest.json` con: target e ID, timestamps de inicio/fin, lista de tools ejecutados (step, tool, status `success`/`failed`/`skipped_no_web`, output), `out_of_scope_findings` y `errors`.
